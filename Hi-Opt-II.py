import math

from Player import Player
from Strategy import Strategy

class Hi_Opt_II(Strategy):

    def __init__(self, name, money, game):
        super().__hit__(name,money, game)
        self.count = 0
        self.true_count_play = 0
        self.true_count_bet = 0
        self.ace_count = 0
        self.total_ace = len(self.game.cards) / 13
        self.total_cards = len(self.game.cards)

    def add_count(self,value):
        if value in [2,3,6,7]:
            self.count += 1
        elif value in [4,5]:
            self.count += 2
        elif value in [8,9]:
            self.count += 0
        elif value == 10:
            self.count -= 2
        elif value == 11:
            self.ace_count += 1

    def calc_true_count(self):
        self.true_count_play = round(self.count / (len(self.game.cards) / 52))
        self.true_count_bet = round((self.count + ((self.total_ace * ((self.total_cards - len(self.game.cards)) / self.total_cards))- self.ace_count))/(len(self.game.cards) / 52))

    def decide_bet(self):
        if self.true_count_bet <= 0:
            bet = 1
        elif self.true_count_bet == 1:
            bet = 2
        elif self.true_count_bet == 2:
            bet = 4
        elif self.true_count_bet == 3:
            bet = 8
        elif self.true_count_bet > 3:
            bet = 12

    def decide_move():
        return

